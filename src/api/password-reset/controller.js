...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your run-forrest account.<br>
        ...
        &mdash; run-forrest Team
      `
      return sendMail({ toEmail: email, subject: 'run-forrest - Password Reset', content })
    })
